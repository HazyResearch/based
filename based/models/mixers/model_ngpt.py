...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.index_decays = nn.Parameter(torch.randn(self.n_head) * 1e-3)
        self.terrace_mix  = nn.Parameter(torch.ones(self.seg_len) * 0.5)
        
        # self.index_decays = nn.Parameter(torch.randn(self.n_head, self.seg_len) * 1e-3)
        # self.mega_decays  = nn.Parameter(torch.randn(self.n_head) * 1e-3)
        self.index_decays = nn.Parameter(
            -0.1 * torch.ones(self.n_head, self.seg_len) + 
            torch.randn(self.n_head, self.seg_len) * 0.01
        )
        self.mega_decays = nn.Parameter(
            -0.1 * torch.ones(self.n_head) + 
            torch.randn(self.n_head) * 0.01
        )
        
        self.use_cylon = config.use_cylon
        # self.noise_eps = 1e-8
        self.noise_eps = 1e-5
    ...
    def forward(self, x):
        ...
        if self.use_cylon:
            ...
            # pad q, k, v to be divisible by segment_size
            seq_len_pre_pad = q.shape[1]
            pad = -1
            if seq_len_pre_pad % segment_size != 0:
                # add padding to the end of the sequence dimension (dim=1)
                pad = segment_size - (seq_len_pre_pad % segment_size)
            ...
            l = segment_size
            m = math.ceil(max(q.shape[1], k.shape[1]) / segment_size)
            ...
            attention_terrace = F.softmax(scores_terrace, dim=-1)
            output_terrace    = torch.einsum('bhnm,bmhd->bnhd', attention_terrace, v).permute(0, 2, 1, 3).reshape(batch_size, n_heads, num_segments, segment_size, d_vo)
            # end terrace
            
            # apply softmax
            attention_weights = F.softmax(scores, dim=-1)
            attention_weights = F.dropout(attention_weights, self.dropout if self.training else 0.0)
            
            attention_weights_chunked = attention_weights.reshape(batch_size, n_heads, num_segments, segment_size, seq_len)
            ...
            mega_decay_const = torch.clamp(torch.exp(self.mega_decays * segment_size).reshape(1, n_heads, 1), min=0.0, max=2.0)
            ...
            for i in range(1, num_segments):
                ...
                # if torch.isnan(to_append).any():
                #     print(f"i: {i}")
                #     print(f"mega_decay_const: {mega_decay_const}")
            ...
            query_chunks     = q.reshape(batch_size, num_segments, segment_size, n_heads, d_qk)
            ...
            terrace_weight = terrace_weight.view(1, 1, 1, segment_size, 1)
...
class GPTConfig:
    ...
    use_cylon: bool = False # Added cylon2 support parameter
    seg_len: int    = 64
