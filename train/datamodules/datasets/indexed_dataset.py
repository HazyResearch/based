...
class MMapIndexedDataset(torch.utils.data.Dataset):
    class Index(object):
        ...
        def __init__(self, path, skip_warmup=False):
            ...
            if not skip_warmup:
                print_rank_zero("    warming up index mmap file...")
                _warmup_mmap_file(path)

            self._bin_buffer_mmap = np.memmap(path, mode="r", order="C")
            self._bin_buffer = memoryview(self._bin_buffer_mmap)
            print_rank_zero("    reading sizes...")
            self._sizes = np.frombuffer(
                self._bin_buffer, dtype=np.int32, count=self._len, offset=offset
            )
            print_rank_zero("    reading pointers...")
            ...
            print_rank_zero("    reading document index...")
    ...
    def _do_init(self, path, skip_warmup):
        ...
        if not skip_warmup:
            print_rank_zero("    warming up data mmap file...")
            _warmup_mmap_file(data_file_path(self._path))
        print_rank_zero("    creating numpy buffer of mmap...")
        self._bin_buffer_mmap = np.memmap(
            data_file_path(self._path), mode="r", order="C"
        )
        print_rank_zero("    creating memory view of numpy buffer...")
        self._bin_buffer = memoryview(self._bin_buffer_mmap)
